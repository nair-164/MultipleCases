...
if {$argc < 7} {
   ...
   set CSR       [lindex $argv 0]; # Cyclic stress ratio   
   set maxCycles [lindex $argv 1]; # Maximum number of cycles
   set maxStrain [lindex $argv 2]; # Cutoff shear strain
   set sigvo     [lindex $argv 3]; # Initial Vertical Stress
   set K0        [lindex $argv 4]; # Lateral earth pressure coefficient
   set alpha     [lindex $argv 5]; # Initial stress ratio 

} elseif {($argc >1) && ($argc < 6)} {
   puts "No enough input parameters $argc"
}
...
# Read PM4Sand material parameters from file
source Material.tcl
